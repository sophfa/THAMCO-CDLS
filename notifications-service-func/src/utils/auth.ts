...
export async function validateToken(
  ...
  try {
    ...
    if (!authHeader) return { isValid: false, error: "No authorization header provided" };
    if (!authHeader.startsWith("Bearer ")) return { isValid: false, error: "Invalid authorization header format" };
    ...
    if (!domain || !audience || !client) {
      try {
        ...
        if (!userId) return { isValid: false, error: "Token missing user ID (sub claim)" };
        if (payload.exp && payload.exp * 1000 < Date.now()) {
          return { isValid: false, error: "Token has expired" };
        }
        ctx.log?.(`Token validated for user: ${userId} (DEV MODE - NO SIGNATURE VERIFICATION)`);
